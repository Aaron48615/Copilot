...
1. 先约定同步、按注册顺序派发，每次订阅有独立记录，允许同一个函数订阅多次。on 和 once 返回单次取消函数，方便精确退订。直接用 `Set<函数>` 会自动去重，语义不同，不能说它完整复刻了 Node.js EventEmitter。
...
4. 这版中同步异常会向调用者传播，并中断剩余监听器，异步回调返回的 Promise 不会被等待。它也没有 Node 的特殊 error 事件和拒绝捕获机制。要隔离异常继续通知，必须显式捕获并报告，不能悄悄吞掉错误。

5. 重复订阅能分别取消、回调中新增和删除监听器、once 的嵌套派发，以及无人订阅时返回 false，这些都要过一遍。组件卸掉还应调用取消函数。规则和清理责任一起明确后，代码虽短，也能解释真实使用中的主要边界。
...
2. 只有提前退订仍不够，因为外层 emit 已经保存了快照。假设 A 在 once B 之前执行，A 嵌套 emit 先消费 B，外层随后仍会走到快照中的 B。fired 标记让这个旧记录被跳过，确保一次订阅最多调用一次。

3. 让 A 只嵌套一次，记录 B 的实际调用次数来验证这个边界，避免测试自身无限递归。一次订阅消费后重新注册同一个函数，应得到新的记录和新标记，那是另一份合法订阅，不能被旧标记永久屏蔽。
...
3. 业务需要等待所有监听器，就另外定义 emitAsync，并明确串行还是并行、一个失败是否继续、返回什么结果。并行收集可结合 allSettled，串行则逐个 await。这些都会改变时序，不能在原同步 API 上不说明就替换行为。
