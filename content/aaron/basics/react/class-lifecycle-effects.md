---
id: aaron-basic-react-class-lifecycle-effects
title: React 类组件生命周期，以及函数组件如何处理副作用
aliases: [请讲讲：React 类组件生命周期，以及函数组件如何处理副作用, 关于“React 类组件生命周期，以及函数组件如何处理副作用”，你会怎样回答？]
category: react
difficulty: 进阶
priority: normal
projects: []
keywords: [类组件, 生命周期, useEffect, 副作用]
---

# React 类组件生命周期，以及函数组件如何处理副作用
...
函数组件里通常用 Hooks 管理状态和副作用，例如用 useEffect 建立订阅并返回清理函数。理解时我会按“和外部系统同步”来组织，而不是把每一个类生命周期机械地替换成一个 Effect。
