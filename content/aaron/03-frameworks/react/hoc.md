...
高阶组件其实不是一个组件，是一个函数：接收一个组件，返回一个新组件，新组件在内部渲染原来的组件并给它加料，比如注入 props、做条件渲染。写出来就是 const withAuth = (Wrapped) => (props) => isLogin ? <Wrapped {...props} /> : <Login />，它把组件的包装逻辑集中到了一个函数里。
