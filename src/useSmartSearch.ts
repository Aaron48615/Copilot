...
export function useSmartSearch(run: (text: string) => void, cancel: () => void, query: string) {
  ...
  useEffect(() => {
    if (automatic && !composing.current && query !== handled.current && /[\p{L}\p{N}]/u.test(query)) {
      timer.current = setTimeout(() => submit(query), 800)
    }
    return () => clearTimeout(timer.current)
  }, [query, revision, automatic])
  return { automatic, stop, submit, edit,
    toggle: () => { stop(); setAutomatic(!automatic); handled.current = '' },
